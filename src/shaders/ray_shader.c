...
/*   Updated: 2024/06/11 20:24:12 by kdaniely         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	scatter(t_control *ctl, t_hitrecord *hr, int bounce)
{
	...
	if (hr->hit->material.properties & METALLIC)
	{
		...
		scattered.direction = unit_vector(\
			sum_vec3(&scattered.direction, &fuzz));
	}
	else if (hr->hit->material.properties & DIELECTRIC)
	{
		...
		scattered.direction = ray_refract(&hr->r.direction, &hr->normal, ri);
	}
